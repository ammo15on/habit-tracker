...
@Composable
fun EditTaskTimerDialog(
  task: HabitTask,
  onDismiss: () -> Unit,
  onConfirm: (targetMinutes: Int) -> Unit
) {
  ...
  AlertDialog(
    ...
    text = {
      Column(modifier = Modifier.fillMaxWidth()) {
        ...
        OutlinedTextField(
          ...
          placeholder = { Text("e.g. 60 (or 0 for no target)") },
          singleLine = true,
        ...
        Text(
          text = "If active time exceeds this target, the extra duration will be highlighted in Red.",
    ...
    confirmButton = {
      Button(
        onClick = {
          val minutes = targetMinutesStr.toIntOrNull() ?: 0
          onConfirm(minutes)
        },
        shape = RoundedCornerShape(12.dp),
